...
applyCapitalization :: Word.IsCapitalized -> [a] -> [(Case, a)]
applyCapitalization _ [] = []
applyCapitalization c (x : xs) = (if c == Word.IsCapitalized then Uppercase else Lowercase, x) : fmap (\y -> (Lowercase, y)) xs
