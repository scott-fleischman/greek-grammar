...
haskellDecomposeCharPath = rootPath </> "haskell" </> "greek-grammar" </> "src" </> "Data" </> "Unicode" </> "DecomposeChar" <.> "hs"
...
\module Data.Unicode.DecomposeChar where\n\
\\n\
...
type FileGenerator = (FilePath, Text -> Text)

generateDecomposeChar :: FileGenerator
generateDecomposeChar = (haskellDecomposeCharPath, toHaskellDecompose)

outputTokenPairs :: FileGenerator
outputTokenPairs = (haskellUnicodeScriptPath, toHaskellScript)

generateFile :: FileGenerator -> IO ()
generateFile (path, convert) = do
  content <- readFile unicodeDataPath
  writeFile path (convert content)
  return ()
