...
function renderGroup(element, items) {
	items.forEach(item => {
		...
		const words = R.take(100, item.words);
		words.forEach(word => {
...
export function render() {
	...
	const doRender = R.curry(renderGroup)(element);
	const callback = R.compose(doRender, process, x => x.items);
	$.getJSON(`data/${dataName}.json`, callback);
}
