...
const Nav = ({stages, currentStage}) => {
  return (
    <nav className="navbar navbar-default">
      <ul className="nav nav-pills">
        <li role="presentation" className="dropdown">
          <a className="dropdown-toggle" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
            <b>Stage</b>: {stages[currentStage].title} <span className="caret"></span>
          </a>
          <ul className="dropdown-menu">
            {R.map(x => <li key={x.index}><a href="#">{x.title}</a></li>, stages)}
          </ul>
        </li>
      </ul>
    </nav>);
...
function renderGroup(groupName, dataName, data) {
  const stageData = data.data.stages;
  ReactDOM.render(
    <Nav stages={stageData} currentStage={0} />,
    document.getElementById('nav'));

  const mainElement = document.getElementById('main');
  const titleData = data.data[groupName];
  var renderedGroups = R.map(x => (<GroupCollapse key={x.key} elementId={mainElement.id} groupName={groupName} dataName={dataName} item={x} title={titleData[x.key].title} />), data.items);
...
export function render() {
  ...
  loadCombinedData(['stages', 'word', 'work']).then(callback);
}
