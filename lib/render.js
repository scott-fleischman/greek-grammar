...
const GroupCollapse = ({elementId, item, title}) => {
...
function renderGroup(element, groupName, data) {
	...
	var rendered = R.map(x => (<GroupCollapse key={x.index} elementId={element.id} item={x} title={titleData[x.key].title} />), data.items);
