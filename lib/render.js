      ...
      <ul className="dropdown-menu">
        { R.flatten(
            ...
            , !R.isNil(focusSource) && focusSource !== focusResult ?
              ...
              [],
            , !R.isNil(maybe(focusResult, focusSource)) ?
      ...
      { R.map(item =>
        {
          ...
          return (
            <div key={maybe(currentGroupIndex, '*') + '.' + itemIndex} className="panel panel-default">
...
export class App extends React.Component {
  ...
  render() {
    ...
    const getGroupTitle = x => x === undefined ? noneTitle : getType(x).title;

    const currentStageIndex = maybe(this.state.currentStage, 0);
    ...
    const currentTypeIndex = this.state.currentType && R.contains(this.state.currentType, stageTypes) ?
      ...
      maybe(focusResult, focusSource, stageTypes[0]);
    ...
    const groupByFunc = currentGroupIndex === undefined ? x => x.valueIndex : (x => maybe(x.propertyValues[currentGroupIndex], 'none'));
    const groupTypeIndex = maybe(currentGroupIndex, currentTypeIndex);
    const groupType = getType(groupTypeIndex);
    const getValueTitle = x => maybe(maybeProp('title', groupType.values[x]), noneTitle);
